...
options.objSize = 24; % size of the face detection template
...
options.finalNegs = 1000; % the minimal number of bootstrapped nonface samples. 
% The training will be stopped if there is no enough nonface samples in the
% final stage. This is also to avoid overfitting.
options.numFaces = Inf; % the number of face samples to be used for training.
% Inf means to use all face samples.
options.numThreads = 18; % the number of computing threads for bootstrapping

options.boostOpt.treeLevel = 8; % the maximal depth of the DQT trees to be learned
...
options.boostOpt.minSamples = 1000; % minimal samples required to continue training. 1000 is preferred in practice
